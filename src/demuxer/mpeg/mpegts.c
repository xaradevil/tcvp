...
mpegts_read_packet(mpeg_stream_t *s)
{
	...
	if(al > 0){
	    ...
	    af->transport_private = getbits(s, 1, "transport_private_flag");
	    ...
	    assert(br >= 0);
	    while(br--){
		assert(getbits(s, 8, "stuffing") == 0xff);
	    }
	}
    }
...
mpeg_open(char *name, conf_section *cs)
{
    ...
    mf->packets = calloc(ms->n_streams, sizeof(list *));
